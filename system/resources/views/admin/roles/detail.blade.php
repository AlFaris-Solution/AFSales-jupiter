...
@section('content')
                    ...
                    {!! Form::label('permission', 'Permissions', ['class' => 'control-label']) !!}
                      <table class="table table-bordered table-condensed">                      
                       @foreach($permissions as $permission)                       
                        <tr>
                          <td style="width: 10px; cursor: not-allowed;">
                          {{ Form::checkbox('permissions[]', $permission->id, $role->permissions, ['class' => 'flat']) }}                            
                          </td>
                          <td>{{ Form::label($permission->name, ucwords(str_replace('_', ' ', $permission->name))) }}</td>
                        </tr>                        
                       @endforeach                    
                      </table>                      
                  </div>
    ...
    <a href="{{route('admin.roles.index')}}" class="btn btn-primary"><i class="fa fa-chevron-left fa-fw"></i> Kembali</a>
    {!! Form::close() !!}
@stop