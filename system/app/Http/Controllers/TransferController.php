...
class TransferController extends Controller
{
    ...
    public function insert(Request $request)
    {
        ...
        $transfer->keterangan=$request->keterangan;
        $transfer->gudang_asal=$request->gudang_asal ?? 'Transfer Antar Gudang';
        ...
        */
        ...
        if ($transfer->save()){
            ...
            foreach ($request->nama_produk as $key => $v) 
            {
                ...
                $gudangId = Gudang::find($request->gudang_asal)['id'] ?? null;
                ...
                $historiStok = HistoriStok::create([
                    'tgl_berubah' => date('Y-m-d H:i:s'),
                    'id_produk' => $v,
                    'nama_produk' => $namaProduk,
                    'keterangan' => "STOK KELUAR DARI GUDANG {$gudangId}",
                ...
                $historiStokTerakhir = $historiStokTerakhir['stok_tersisa'] ?? 0;
                $historiStok = HistoriStok::create([
                    'tgl_berubah' => date('Y-m-d H:i:s', strtotime("+3 second", strtotime(date('Y-m-d H:i:s')))),
                    'id_produk' => $v,
                    'nama_produk' => $namaProduk,
                    'keterangan' => "STOK MASUK KE GUDANG {$gudangId}",
                    'gudang_asal' => $request->ke_gudang[$key],
                    'no_so' => null,
