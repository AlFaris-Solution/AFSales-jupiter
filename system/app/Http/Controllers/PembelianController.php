...
class PembelianController extends Controller
{
    ...
    {
        $uoms =  Uom::All();
        $suppliers = Kontak::where('tipe', 'Supplier')->get();

        return view('modules.pembelian.index', compact('uoms', 'suppliers'));
    }

    public function hutang()
    {
        return view('modules.pembelian.hutang');
    }
    ...
    public function data()
    {
        ...
        if (!empty(request('supplier_id')) && !empty(request('start_date')) && !empty(request('end_date'))) {
            $roles = $roles
                ->where('kontak_id', request('supplier_id'))
                ->whereBetween('pembelians.tanggal_faktur', [$tglAwal, $tglAkhir]);
        }
        ...
        foreach ($roles as $key => $list) {
            ...
            $row['no_po']               = $list->no_po;
            $row['nama_kontak']         = $list->nama_kontak;
            $row['total_setelah_pajak'] = $list->total_setelah_pajak;
            $row['saldo_terutang']      = $list->saldo_terutang;
            $row['action'] = '<div class="text-center btn-group btn-group-justified"><a href="pembelian/detail/'.$list->id.'" title="Detail Data Pembelian"><button type="button" class="btn btn-sm btn-success"><i class="fa fa-eye"></i></button></a> <a href="#" style="display: none;" title="Print Data Pembelian"><button type="button" class="btn btn-sm btn-default"><i class="fa fa-print"></i></button></a> <a href="pembelian/delete/'.$list->id.'" title="Delete" Onclick="return ConfirmDelete();"><button type="button" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></button></a></div>';
            $data[] = $row;
        }
        ...
        if ($pembelian->save()){
            ...
            foreach ($request->nama_produk as $key => $v)
            {
                ...
                $historiStok = HistoriStok::create([
                    ...
                    'keterangan' => $pembelian->keterangan ?? 'PEMBELIAN',
    ...
    */
    ...
    {   /*
        ...
        */
        ...
        $pembelianDetails = PembelianDetail::where('pembelian_id', $id)->get();

        foreach ($pembelianDetails as $key => $detail) {
            $historiStokTerakhir = HistoriStok::where('id_produk', $detail->kode_produk)->latest('tgl_berubah')->first();
            $historiStokTerakhir = $historiStokTerakhir['stok_tersisa'] ?? 0;
            $produk  = Product::find($detail->kode_produk)->first();
            $historiStok = HistoriStok::create([
                'tgl_berubah' => Carbon::now(),
                'id_produk' => $produk->id,
    ...
    public function getdataHutang(){
        $roles = DB::table('pembelians')
                ...
                ->leftJoin('kontaks','pembelians.kontak_id','=','kontaks.id')
                // ->select('pembelians.*', 'kontaks.nama_kontak','kontaks.kurs','pembelians.saldo_terutang AS totalhutang', DB::raw('SUM(IFNULL(pembayarans.nilai, 0)) AS totalbayar'), DB::raw('(IFNULL(pembelians.saldo_terutang, 0)) - (SUM(IFNULL(pembayarans.nilai, 0))) AS saldohutang')
                // )
        ...
        if (!empty(request('start_date')) && !empty(request('end_date'))) {
            $roles = $roles->whereBetween('pembelians.tanggal_faktur', [$tglAwal, $tglAkhir]);
        }
        // if (request('tgl_jatuh_tempo') == 1) {
        //     $roles = $roles->where('pembelians.tgl_jatuh_tempo', '<=', Carbon::now());
        // }
        ...
        foreach ($roles as $key => $list) {
            $row = array();
            if (request('tgl_jatuh_tempo') == 1) {
                if (strtotime($list->first()->tgl_jatuh_tempo) <= strtotime(date('Y-m-d', strtotime(Carbon::today())))) {
                    $row['tanggal_faktur'] = date('d/m/Y', strtotime($list->first()->tanggal_faktur));
                    $row['tgl_jatuh_tempo'] = date('d/m/Y', strtotime($list->first()->tgl_jatuh_tempo));
                    $row['nama_kontak'] = $list->first()->nama_kontak;
                    $row['kurs'] = $list->first()->kurs;
                    $row['totalhutang'] = $list->first()->saldo_terutang;
                    $row['totalbayar'] = $list->sum('nilai') ?? 0;
                    $row['saldohutang'] = ($list->first()->saldo_terutang - $list->sum('nilai')) ?? 0;
                    $row['action'] = '
                        <div class="text-center btn-group btn-group-justified">
                            <a href="pembelian/detail/'.$list->first()->nama_kontak.'" title="Detail Data Pembelian"><button type="button" class="btn btn-sm btn-success">
                            <i class="fa fa-search"></i></button></a> <a href="pembelian/print/'.$list->first()->nama_kontak.'" title="Print Data Pembelian">
                            <button type="button" class="btn btn-sm btn-default"><i class="fa fa-print"></i></button></a> <a href="pembelian/delete/'.$list->first()->nama_kontak.'" title="Delete" Onclick="return ConfirmDelete();">
                            <button type="button" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></button></a>
                        </div>';
                    $data[] = $row;
                }
            } else {
